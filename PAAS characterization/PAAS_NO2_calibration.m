...
% Quantum yield:
%   0.3 (Troe et al. 2000)
% Set to zero if quantum yield should not be corrected
yield = 0;
...
% Expand ~ in freq_scan path if present
if isfield(cfg,'freq_scan') && startsWith(cfg.freq_scan,'~')
    homeDir = char(java.lang.System.getProperty('user.home'));
    cfg.freq_scan = fullfile(homeDir,cfg.freq_scan(3:end));
end
...
if isfield(cfg,'do_frequency_correction')
    do_frequency_correction = cfg.do_frequency_correction;
else
...
%% Plot timeseries
figure('Units','centimeters','Position',[26 20 22 15])
for i = 1:n_laser
    temp = laser_data{1,i};
...
%% Ramped calibration
if strcmp(cfg.calibration_type,'ramped')

figure('Units','centimeters','Position',[2 3.5 46 17])
tiledlayout(1,2,'TileSpacing','compact','Padding','compact')

% Initialize data dump variable
